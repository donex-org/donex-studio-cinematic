    ...
    <header className="fixed top-0 left-0 right-0 z-50">
      <div
        className={`transition-all duration-500 ease-out ${
          isScrolled ? "pt-2 px-4" : "pt-0 px-0"
        }`}
      >
        <nav
          className={`transition-all duration-500 ease-out ${
        ...
        >
          ...
          >
            ...
            <Link to="/" className="flex items-center gap-2">
              <div
                className={`rounded-full bg-blue-500/20 backdrop-blur-sm border border-blue-400/30 flex items-center justify-center transition-all duration-500 hover:scale-110 ${
            ...
            >
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  to={link.href}
                  className={`relative font-medium transition-all duration-500 group overflow-hidden ${
                ...
                >
                  ...
                  <span
                    className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-0.5 bg-blue-400 transition-all duration-300 ${
                      isActive(link.href)
                        ? "w-3/4"
              ...
              <div className="px-2 pt-2 pb-3 space-y-1">
                {navLinks.map((link) => (
                  <Link
                    ...
                    className={`w-full block text-left hover:bg-white/5 px-3 py-2 font-medium transition-all duration-200 rounded-lg ${
                      isActive(link.href)
